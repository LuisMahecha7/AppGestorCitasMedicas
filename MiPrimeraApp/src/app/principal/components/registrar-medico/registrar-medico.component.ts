...
export class RegistrarMedicoComponent {
  ...
  private registerDoctor(nombres: string, primerApellido: string, segundoApellido: string, especialidad: string, celular: string, direccion: string, email: string, password: string) {
    // Enviar los datos al backend para registrar al médico
    this.http.post('http://localhost/tu-backend/register-doctor.php', {
      ...
      .subscribe(
        (response: any) => {
          if (response.status === 'email_exists') {
            ...
            this.registerDoctor(nombres, primerApellido, segundoApellido, especialidad, celular, direccion, email, password);
          }
        },
        error => {
